...
src/
  main/
    ...
    ipc/
      ...
      roadmap.ts          roadmap:read → GET /api/architects/:key/roadmap[?view=&id=&depth=] (native roadmap tab; read-only — no write handler, all mutations go through architect MCP)
  ...
  renderer/src/
    ...
    state/
      ...
      roadmapStore.ts     Zustand store — window-lifetime roadmap pane view state (Current/Archive, selected item, drilled-into archive root, collapsed nodes); deliberately not persisted
    hooks/
    ...
    pages/
      ...
      architect-window/
        ...
        components/              RightPane, BottomTabs, MainTerminalStack,
                                 ...
                                 Roadmap/ (native roadmap tab — RoadmapPane, RoadmapHierarchy
                                 (collapsible current/archive-subtree tree), RoadmapDetail
                                 (kind/status/outcome/success-criteria/dependencies/children/
                                 linked-ticket-evidence panel), RoadmapArchiveList (compact
                                 archived-root summaries), roadmapTree.ts (pure
                                 parent_id/order graph → flattened visible rows)),
...
There are exactly 8 tab types (`SessionTab.type`): kanban, roadmap, event-log, ticket, terminal, git-diff, files, browser. All render directly in `RightPane` via hardcoded panels — none go through a generic content lookup. `src/renderer/src/plugins/registry.ts` holds a small `type → icon` map (`getTabPlugin(type)`) with two live consumers: **`RightPane.mapTabToBarTab`** looks up the icon component for the `TabBar` (unknown types return `null`, filtered out of the tab bar), and `keys/dispatcher.ts`'s `getRightTabIds()` uses the same lookup to validate which tab IDs are cyclable via keyboard (Cmd+2..9, focus-left/right/up/down) — both are just existence checks against the fixed set of known types.

`roadmap` is a **review-only** tab over the daemon's architect roadmap (`GET /api/architects/{key}/roadmap`) — the desktop never mutates it; every create/edit/move/archive/restore happens through the architect's own MCP tools. `RoadmapPane` (`pages/architect-window/components/Roadmap/`) owns its own fetch, independent of session state (the roadmap is architect-scoped, not per-session, the same way the kanban board is): it fetches on tab activation, and refetches on the daemon's `roadmap_updated` architect-stream reason (debounced 500ms so a burst of MCP mutations collapses into one refetch) and on `stream_connected` (no debounce — a reconnect has no backlog, so it must reconcile immediately). A manual refresh button/keybinding covers the rest. A `Current | Archive` header switch (`state/roadmapStore.ts`, window-lifetime, unpersisted) toggles between the live roadmap and a compact archived-root list; selecting an archived root drills into that entry's full preserved subtree via the same hierarchy component, with its own "← archived roots" back step. `roadmapTree.ts` builds/flattens the flat `parent_id`/`order` item graph into visible rows the same way `gitDiffTree.ts` does for changed files (kept as a separate lowercase-prefixed utility file — do not add a component literally named `RoadmapTree.tsx` alongside it, since macOS's case-insensitive filesystem resolves the two to the same file). Wide layouts (`ResizeObserver`-measured, `WIDE_MIN_WIDTH = 640` — same threshold as `FilesPane`) split hierarchy/detail; narrow layouts drill in with a back button, mirroring `FilesPane`'s idiom. Selecting a linked ticket calls the same `onTicketReference` callback `TicketWorkflow` uses for in-body ticket references (threaded through `RightPane`), opening the existing `TicketDetail` modal — the roadmap never renders its own ticket UI. A linked ticket id or roadmap dependency id missing from the daemon's response renders an inline "not found" row rather than crashing or silently dropping the link. Keyboard bindings (`j/k/l/h/o`, `g g`/`shift+g`, jump-up/down, `r` refresh, `a` toggle Current/Archive) live in `keys/paneBindings.ts`'s `ROADMAP_BINDING_DEFAULTS`, layered under any daemon `roadmap` shortcut config the same way `files`/`git-diff` are (not a required section). A `lab/roadmap-layout/` standalone Vite harness (mirrors the `git-diff` sibling repo's `lab/live-diff` convention) validated the wide/narrow hierarchy-detail interaction against static fixtures before this pane was wired to the daemon.
...
- **Independent consumers** — `useArchitectData` re-fetches `architects.get(architectKey)` (metadata/repos) and `tickets.list(architectKey)` (board state) on any daemon event; `useArchitectSessionDiscovery` re-syncs the session store on the session reasons; `RoadmapPane` independently subscribes the same way to refetch on `roadmap_updated`. None of them know about each other — see [Native tab registry](#native-tab-registry) for the roadmap pane's debounce.
- **Event shape** — `ArchitectEvent` from `@hiveryn/shared/domain` (not mirrored locally): `{ type: 'workspace_changed', architect_key, reason, ticket_id, session_id, at }`. Reasons are `ticket_created`/`ticket_updated`/`ticket_moved`/`ticket_deleted`/`ticket_concluded`, `session_started`/`session_ended`, and `roadmap_updated` (any successful roadmap mutation — MCP or, in future, desktop-originated); the session reasons carry `session_id`, the others carry empty `ticket_id`/`session_id`. The desktop-local `StreamConnectedEvent` (`src/shared/types.ts`) is unioned with it as `ArchitectStreamEvent`.
