...
5. **Approval inputs.** An intent may carry `inputs` (a bounded schema: `text`, `textarea`, `choice`, `boolean`; see `@hiveryn/shared/domain` `IntentInputField`) and `unresolved_inputs`, both parsed from `raw` by `intentInputsModel.ts` (throws on a malformed schema). `IntentInputsForm` renders them between the details and the actions, prefilled with usable defaults (a stale choice default starts unselected). Approve runs the client-side mirror checks first (showing per-field errors), then submits every field's value; the daemon is the authority, and its 400 is shown inline via `ApiEnvelopeError` with the card and values left editable for correction. When `unresolved_inputs` is non-empty under an allow policy the daemon will not approve automatically, so the countdown reads "needs input" and a notice lists the unresolved fields. Deny never reads or validates the inputs.
