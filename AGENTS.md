...
src/
  main/
    ...
    ipc/
      ...
      actions.ts          actions:open-window (optional session id → `actions:open-session` to the window)/list/get/launch/runs/run/cancel → /api/actions*, /api/action-runs*; actions:events:subscribe/unsubscribe → the global actions SSE stream (daemon/action-events.ts)
...
- **Actions rows** — `buildRows` appends an `{ kind: 'actions' }` row that opens the Actions window (its home stays reachable), followed by one `action-run` row per running execution with a session (`runningActionRuns` over `actions.runs`), shaped like an architect and its sessions: a query matching "Actions" keeps every execution, otherwise the Actions row heads the executions matching by action name or prompt. The palette refetches them on show, on its poll and on every actions-stream event while visible. Selecting an execution calls `actions:open-window` with its session id; main opens/focuses the window and, once loaded, sends `actions:open-session`, which `useOpenSessionRequest` applies once that session reaches the window's store (retrying like `usePaletteSessionSwitch`, pushing to the error center if it never appears).
